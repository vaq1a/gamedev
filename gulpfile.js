  ...
  return gulp
    ...
    .pipe(
      index({
        ...
        'prepend-to-output': () =>
          `<head> <title>All pages</title><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=2.0"><link rel="shortcut icon" href="favicon.ico"></head><body>`,
        title: 'All pages',
        'title-template': (title) => `<h1 class="` + pref + `__title">${title}</h1>`,
        'section-template': (sectionContent) =>
          `<section class="` + pref + `__section"> ${sectionContent}</section>`,
        'section-heading-template': (heading) =>
          `<!--<h2 class="` + pref + `__section-heading">${heading}</h2>-->`,
        ...
        'item-template': (filepath, filename) =>
          ...
          `__item"><a class="` +
          pref +
          `__item-link" href="./${filename}">${filename}</a></li>`,
...
gulp.task('normalize', function () {
  return gulp
    .src('src/libs/normalize-scss/sass/**/*.+(scss|sass)')
...
gulp.task('copyJqueryToJs', function () {
  return gulp.src(['src/libs/jquery/dist/jquery.min.js']).pipe(gulp.dest('src/js'));
});
/**
...
gulp.task('browserSync', function (done) {
  ...
  browserSync
    .watch(['src/*.html', 'src/js/**/*.js', 'src/includes/**/*.json', 'src/includes/**/*.svg'])
...
gulp.task('sassCompilationForDist', function () {
  return gulp
    ...
    .pipe(replace('@charset "UTF-8";', ''))
    .pipe(
      autoprefixer(['last 6 versions', '> 1%', 'ie >= 9', 'and_chr >= 2.3'], {
        cascade: true,
      }),
    )
...
gulp.task(
  ...
  [
    ...
    'html:buildAllPages',
    'copyImgToDist',
    'sassCompilationForDist',
    'mergeCssLibs',
    'createCustomModernizr',
    'copyLibsScriptsToJs',
  ],
  function () {
    gulp.src(['src/ajax/**/*']).pipe(gulp.dest(path.dist + '/ajax'));

    gulp.src(['src/video/**/*']).pipe(gulp.dest(path.dist + '/video'));

    gulp.src('src/fonts/**/*').pipe(gulp.dest(path.dist + '/fonts'));
    ...
    gulp.src('src/assets/**/*').pipe(gulp.dest(path.dist + '/assets'));

    gulp
      .src(['!src/__*.html', '!src/~*.html', '!src/_tpl_*.html', '!src/_temp_*.html', 'src/*.html'])
